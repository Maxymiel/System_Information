...
namespace ConverterHTML
{
    public partial class Converter : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            Thread thread = new Thread(new ParameterizedThreadStart(Cvrt));
            thread.Start(files);
        }
        ...
        private void Cvrt(object files)
        {
            ...
            foreach (string file in (string[])files)
            {
                try
                {
                    ...
                    string colorU = "e6e6e6";
                    string LAA = "";
                    if (comp.LoginAsAdministrator) { colorU = "ffbec7"; LAA = " (Администратор локальной машины)"; }
                    ...
                    listhtml.Add("<tr><td width=120 align=right>Описание</td><td bgcolor=e6e6e6 width=65%>" + comp.Description + "</td><td bgcolor=e6e6e6 align=center> " + comp.GenDate.ToString("dd.MM.yyyy HH:mm:ss") + " </td></tr>");
                    ...
                    if (!comp.IsVirtualMachine)
                    {
                        try
                        {
                            ...
                            double totalMemMark = -3 + (avgmem / 177);
                            ...
                            double totalCPUmark = comp.CPUs.Sum(t => t.MaxClockSpeed) / 1000 * comp.CPUs.Sum(t => t.NumberOfCores) * 1.666666666666667;
                            double totalDriveMark = 25;

                            totalCPUmark = Math.Round(totalCPUmark, 2);

                            Drive drive = comp.Drives.Find(t => t.IsPrimary);
                            ...
                            double[] tempvlv2 = levelP2(totalCPUmark, totalDriveMark, comp.Marks.GraphicsScore, totalMemMark, comp.RAMArray.RAMCapacity, comp.Drives.Sum(t => (long)t.Size));

                            string LVLtoStr = GeTotaltMarkFromLVL2(tempvlv2[0]);
                            ...
                            listhtml.Add("<tr><th rowspan=6><img src=" + "\"../source/" + Math.Round(tempvlv2[0] - 1).ToString() + ".png" + "\" width=\"50px\" align=" + "\"bottom" + "\" border=" + "\"0" + "\"><br>" + LVLtoStr + "</th>");
                            listhtml.Add("<td align=right>Оценка CPU</td><td width=40% align=center bgcolor=" + getcolorOfLVL2(tempvlv2[1], 1) + ">" + Math.Round(ReturnListMarks(tempvlv2)[1], 2) + "</td></tr>");
                            listhtml.Add("<tr><td align=right>Оценка диска</td><td align=center bgcolor=" + getcolorOfLVL2(tempvlv2[2], 2) + ">" + Math.Round(ReturnListMarks(tempvlv2)[2], 2) + "</td></tr>");
                            listhtml.Add("<tr><td align=right>Оценка GPU</td><td align=center bgcolor=" + getcolorOfLVL2(tempvlv2[3], 3) + ">" + Math.Round(ReturnListMarks(tempvlv2)[3], 2) + "</td></tr>");
                            listhtml.Add("<tr><td align=right>Оценка RAM</td><td align=center bgcolor=" + getcolorOfLVL2(tempvlv2[4], 4) + ">" + Math.Round(ReturnListMarks(tempvlv2)[4], 2) + "</td></tr>");
                            ...
                            listhtml.Add("<tr><td align=right>Размер диска</td><td align=center bgcolor=" + getcolorOfLVL2(tempvlv2[6], 6) + ">" + comp.Drives.Sum(t => (long)t.Size) + " ГБ</td></tr>");
                            listhtml.Add("</table>");
                        }
                        catch
                        {
                    ...
                    listhtml.Add("<td><table width=100% border=0>");
                    listhtml.Add("<tr><td width=120 valign=top align=right>Материнская плата</td><td bgcolor=e6e6e6>" +
                        comp.Motherboard.Manufacturer + "<br>" + comp.Motherboard.Product + "<br>" +
                    ...
                    {
                        listhtml.Add(cpu.Name + "<br>" +
                        "Сокет: " + Helpers.GetSocket(cpu.UpgradeMethod) + "<br>" +
                        "Максимальная частота: " + cpu.MaxClockSpeed + " ГЦ<br>" +
                        "Число ядер: " + cpu.NumberOfCores + "<br>" +
                        "S/N: " + cpu.ProcessorId);
                        listhtml.Add("<hr>");
                    }
                    ...
                    listhtml.Add("<tr><td width=120 valign=top align=right>Видеоадаптер</td><td bgcolor=e6e6e6>" + comp.VideoAdapter.AdapterCompatibility + " " + comp.VideoAdapter.Name + "<br>" +
                        comp.VideoAdapter.AdapterRAM + " МБ.<br>" +
                        comp.VideoAdapter.CurrentHorizontalResolution + "x" + comp.VideoAdapter.CurrentVerticalResolution + "</td></tr>");
                    ...
                    listhtml.Add("<table width=100%> <tr><td width=20 align=left><a href=\"javascript:CloseSubDiv(300)\"><img src=\"../source/close_all.png\" title=\"Свернуть все\" width=\"16px\"></a></td>");
                    listhtml.Add("<td><hr  width=100%></td><td width=20 align=right><a href=\"javascript:CloseSubDiv(300)\"><img src=\"../source/close_all.png\" title=\"Свернуть все\" width=\"16px\"></a></td>");
                    ...
                    foreach (var device in comp.Devices)
                    {
                        try
                        {
                            listhtml.Add("<tr><td width=20></td><td  bgcolor=" + getcolor() + ">  <img src=\"../source/hard.png\" border=\"0\" width=15px height=15px>" + device.Caption + " | Описание: ");
                    ...
                    {
                        listhtml.Add("<tr><td width=20></td><td  bgcolor=" +
                            getcolor() + "><img src=\"../source/soft.png\" border=\"0\" width=15px height=15px>" +
                             app.DisplayName + " | " + app.DisplayVersion + " | " + app.Publisher + "</td></tr>");
                    }
                    listhtml.Add("</table></div><br>");
            ...
            foreach (var comp in GenList)
            {
                ...
                string LVLtoStr = "UNKNOWN";

                if (comp.Marks != null && !comp.IsVirtualMachine) { LVLtoStr = GeTotaltMarkFromLVL2(comp.Marks.TotalMark); }
                else if (comp.IsVirtualMachine) { LVLtoStr = "VIRTUAL"; }

                Svod.Add("<tr bgcolor=" + scolor + ">" +
                    "<td><a href=\"" + comp.MachineName + ".htm\" id=\"" + comp.MachineName + "\" class=\"R_menu\">" + comp.MachineName + "</a></td>" +
                    "<td>" + comp.UserName + "</td>" +
                    "<td>" + comp.WinVersion + "</td>" +
                    "<td>" + comp.Description + "</td>" +
                    "<td><center>" + comp.RAMArray.RAMCapacity + "</center></td>" +
                    "<td bgcolor=\"" + smartcolor + "\"><center>" + SMARTStatus + "</center></td>" +
                    "<td bgcolor=\"" + getcolorOfMark(LVLtoStr) + "\"><center>" + LVLtoStr + "</center></td>" +
        ...
        void GroupGen_sub(string NameGroup, Dictionary<string, List<General>> GenList, string iconpath)
        {
            ...
            {
                Svod.Add("<tr bgcolor=\"#e6e6e6\" width=\"100%\">\r\n<td width=\"90%\">\r\n<a href=\"javascript:shiftSubDiv(" + subdiv.ToString() + ")\" title=\"Развернуть/Свернуть\">\r\n" +
                    "<img id=\"image" + subdiv.ToString() + "\" src=\"../source/plus.png\" title=\"\">\r\n<img src=\"" + iconpath + "\" title=\"\" width=\"32px\" height=\"32px\">\r\n<b>" + item.Key + "</b>\r\n</a>\r\n<i>" + item.Value.Count() + "</i>\r\n</td>\r\n</tr>");

                Svod.Add("<tr>\r\n<td align=\"right\" colspan=\"2\"><div id=\"subDiv" + subdiv.ToString() + "\" style=\"display:none\"><table width=\"98%\">\r\n<tbody>");
                Svod.Add("<tr align=center bgcolor=#c8c8c8><td width=20%><b>Имя компьютера</b></td><td width=12%><b>Пользователь</b></td><td width=20%><b>Версия Windows</b></td><td width=15%><b>Описание</b></td><td width=6%><b>RAM</b></td><td width=7%><b>SMART</b></td><td width=10%><b>Оценка</b></td><td width=10%><b>Время отчета</b></td></tr>");
        ...
        string SMARTVerdict(Dictionary<int, Smart> SMART)
        {
        ...
        static double[] levelP2(double CPUScoreF, double DiskScoreF, float GraphicsScoreF, double MemoryScoreF, float TotalmemF, float TotalDrivesSizeF)
        {
            CPUScoreF = CPUScoreF * 12 / 30;
            DiskScoreF = DiskScoreF * 12 / 50;
            MemoryScoreF = MemoryScoreF * 12 / 20;

            float memoryStand = 8192f;
            float disksizeStand = 220f;

            float GraphicsScore = GraphicsScoreF + 0.1f;

            float TotalmemMark = (100f / (memoryStand / TotalmemF)) / 10f;
            float TotalDrivesMark = 0;
            float TotalDrivesSize = TotalDrivesSizeF;
            ...
            double[] obshLVLMass = new double[7];
        ...
        static string GeTotaltMarkFromLVL2(double mark)
        {
            if (mark < 3) { return "1"; }
        ...
        static string getcolorOfLVL2(double LVL, int ind)
        {
            if (ind == 3) { LVL = (LVL * 12) / 10; }
            else if (ind == 5) { LVL = (LVL * 12) / 10; }
        ...
        static string getcolorOfMark(string LVL)
        {
        ...
        static double[] ReturnListMarks(double[] LVL)
        {
            return new double[] { LVL[0], LVL[1], LVL[2],
        ...
        {
            if (colch) { colch = false; return "E6E6FA"; }
            else { colch = true; return "F0FFFF"; }
        }
    }
}